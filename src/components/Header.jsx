...
const Header = () => {
  ...
  const DesktopMenu = menu.map((men)=>{
    return <div key={men.icon} className='flex items-center gap-4 p-2 cursor-pointer hover:underline underline-offset-8'>
              {men.icon}<p className=''>{men.option}</p>
          </div>
  })
  const MobileMenu = menu.map((menu,ind)=>{
    return ind<3 &&<div key={ind} className='flex items-center gap-4 p-2 cursor-pointer hover:underline underline-offset-8'>
...
const DropMenu = menu.map((menu,ind)=>{
  ...
  return (
    <div className='m-5 md:mx-10 flex items-center justify-between text-gray-300'>
      ...
      <img className='w-[40px] rounded-full border cursor-pointer' src={profileImage} alt='profile'/>
    </div>
  )
}
