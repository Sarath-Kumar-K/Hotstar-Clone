...
const MovieList = ({itemId}) => {
  ...
  const Movies = movieList.map(item=>{
    return <img key={item.poster_path} src={ImageBaseUrl+item.poster_path} className='hover:border-4 hover:shadow-lg shadow-black border-gray-300 transition-all duration-200 ease-in-out w-[300px] h-[300px] md:h-[400px] mr-5 rounded-lg object-cover object-left-top' alt="movie"/>
  })
